...
void NCursesDisplay::DisplayProcesses(std::vector<Process>& processes,
  ...
  for (int i = 0; i < num_processes; ++i) {
    mvwaddstr(window, ++row, pid_column, to_string(processes[i].Pid()).c_str());
    mvwaddstr(window, row, user_column, processes[i].User().c_str());
    float cpu = processes[i].CpuUtilization() * 100;
    mvwaddstr(window, row, cpu_column, to_string(cpu).substr(0, 4).c_str());
    mvwaddstr(window, row, ram_column, processes[i].Ram().c_str());
    mvwaddstr(window, row, time_column,
              Format::ElapsedTime(processes[i].UpTime()).c_str());
    // mvwprintw(window, row, command_column,
    //           processes[i].Command().substr(0, window->_maxx - 46).c_str());
    mvwaddstr(window, row, command_column,
